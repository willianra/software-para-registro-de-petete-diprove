<?php 
require_once "../modelos/Peteteobservado.php";


$peteteobservado=new Peteteobservado();

$idpeteteobservado=isset($_POST["idpeteteobservado"])? limpiarCadena($_POST["idpeteteobservado"]):"";
...
switch ($_GET["op"]){
	...
	// $vehiculo1=SavedImage($vehiculo1,$_POST["origenactual"]);
		...
		if (empty($idpeteteobservado)){
			$rspta=$peteteobservado->insertar($clase,$marca,$tipo,$color,$modelo,$origen,$vehiculo1,$vehiculo2,$vehiculo3,$vehiculo4,$chasis,$plaqueta,$motor,$vin,$otros);
			echo $rspta ? "peteteobservado registrado".$clase.' - '.$marca.' - '.$tipo.' - '.$color.' - '.$modelo.' - '.$origen.' - '.$vehiculo1.' - '.$vehiculo2.' - '.$vehiculo3.' - '.$vehiculo4.' - '.$chasis.' - '.$plaqueta.' - '.$motor.' - '.$vin.' - '.$otros: "peteteobservado no se pudo registrar".$clase.' - '.$marca.' - '.$tipo.' - '.$color.' - '.$modelo.' - '.$origen.' - '.$vehiculo1.' - '.$vehiculo2.' - '.$vehiculo3.' - '.$vehiculo4.' - '.$chasis.' - '.$plaqueta.' - '.$motor.' - '.$vin.' - '.$otros;
		}
		else {
			$rspta=$peteteobservado->editar($idpeteteobservado,$clase,$marca,$tipo,$color,$modelo,$origen,$vehiculo1,$vehiculo2,$vehiculo3,$vehiculo4,$chasis,$plaqueta,$motor,$vin,$otros);
			echo $rspta ? "petete actualizado" : "petete no se pudo actualizar";
		}
	break;

	case 'desactivar':
		$rspta=$peteteobservado->desactivar($idpeteteobservado);
 		echo $rspta ? "petete Desactivado" : "petete no se puede desactivar";
	break;

	case 'activar':
		$rspta=$peteteobservado->activar($idpeteteobservado);
 		echo $rspta ? "petete activado" : "petete no se puede activar";
	break;

	case 'mostrar':
		$rspta=$peteteobservado->mostrar($idpeteteobservado);
 		//Codificar el resultado utilizando json
 		echo json_encode($rspta);
	break;

	case 'listar':
		$rspta=$peteteobservado->listar();
 		//Vamos a declarar un array
 		$data= Array();
		 $url='../vistas/mostrar_observado.php?id=';
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>($reg->condicion)?'<button class="btn btn-warning" onclick="mostrar('.$reg->idpeteteobservado.')"><i class="fa fa-pencil"></i></button>'.
 					' <button class="btn btn-danger" onclick="desactivar('.$reg->idpeteteobservado.')"><i class="fa fa-close"></i></button>'.'<a target="_blank" href="'.$url.$reg->idpeteteobservado.'"> <button class="btn btn-info"><i class="fa fa-eye"></i></button></a>':
 					'<button class="btn btn-warning" onclick="mostrar('.$reg->idpeteteobservado.')"><i class="fa fa-pencil"></i></button>'.
 					' <button class="btn btn-primary" onclick="activar('.$reg->idpeteteobservado.')"><i class="fa fa-check"></i></button>'.'<a target="_blank" href="'.$url.$reg->idpeteteobservado.'"> <button class="btn btn-info"><i class="fa fa-eye"></i></button></a>',
 				"1"=>$reg->idpeteteobservado,
 				"2"=>$reg->clase,
 				"3"=>$reg->marca,
 				"4"=>$reg->tipo,
 				"5"=>$reg->color,
